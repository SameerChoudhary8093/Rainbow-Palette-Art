...
import { supabase } from "@/lib/supabase"; // Yaha humne apna DB connect kiya!
import Link from "next/link";

// Asli Database ka Type
...
export default function MarketplacePage() {
  ...
  // JAISE HI PAGE LOAD HOGA, SUPABASE SE DATA AAYEGA
  useEffect(() => {
    async function fetchArtworks() {
      try {
        const { data, error } = await supabase
          ...
          .select('*')
          .order('created_at', { ascending: false }); // Naya sabse upar
  ...
  const simulatePayment = async () => {
    ...
    try {
      ...
      // Update local state to mark this item as unavailable immediately
      setArtworks((prev) =>
        prev.map((art) =>
      ...
      alert(err?.message || "Failed to update artwork availability in database.");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Back Link */}
        <div className="mb-6">
          <Link href="/" className="inline-flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-indigo-600 transition">
